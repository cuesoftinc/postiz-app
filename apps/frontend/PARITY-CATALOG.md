...
- 2026-08-10 - needs-work pages wave (Task B: public preview, missing-release,
  ...
  degrade to the widget default if Chatbase changes its DOM. support.scss was
  left behind holding only a pointer comment and was DELETED 2026-08-13 (it
  declared zero rules and emitted no CSS); its one durable warning - duplicate
  !important rules on the same ids resolve by document order, so these
  overrides must not live in an unconditionally-loaded global stylesheet - now
  lives in the support.tsx docblock, and the `import './support.scss'` went
  with the file in the same change. tsc clean.
...
| Onboarding wizard (welcome modal, connect steps) | `onboarding/onboarding.tsx`, `onboarding.modal.tsx` | POSTIZ-ONLY-KEEP | Shows for invited teammates; neutral styling; sweep tokens opportunistically. `github.onboarding.tsx` DELETED 2026-08-13: `GithubOnboarding` had exactly one repo-wide occurrence, its own declaration - the wizard never rendered a GitHub step here. `settings/github.component.tsx`, the thing it wrapped, is untouched |
...
| Testimonials side panel on auth | **DELETED 2026-08-13** | N/A-INTERNAL | The whole rail is gone: `auth/testimonial.component.tsx`, `auth/testimonial.tsx`, the shared `react-shared-libraries/src/helpers/testomonials.tsx` fixture (note the upstream typo) and `public/auth/avatars/` (17 marketing headshots, 176 KB). It lost its mount when auth became the centered card - `app/(app)/auth/layout.tsx` renders the brand lockup + card and nothing else, where upstream still mounts `TestimonialComponent` beside the form - so it had zero importers and shipped 17 images of people unconnected to Cuesoft. Recoverable from git history; re-adopting it would mean writing our own quotes anyway |
